...
def analyze_leave_data(csv_file):
    ...
    for collaborateur in stats.keys():
        ...
        # Dictionnaire des jours avec leur type
        jours_type = {}
        total_jours = 0
        ...
        for _, row in collab_df.iterrows():
            ...
            if is_conges_am or is_conges_pm:
                ...
                jours_type[date] = 'AUTRE'  # Présent ou télétravail

        # Calculer la plus longue séquence de jours consécutifs
        # Les week-ends ne cassent PAS la séquence mais ne comptent pas dans le total
        max_consecutifs = 0
        ...
        while current_date <= date_fin:
            ...
            if jour_type == 'CONGES':
                # Jour de congés/RTT - incrémente le compteur
                current_consecutifs += 1
                max_consecutifs = max(max_consecutifs, current_consecutifs)
            elif jour_type == 'WEEKEND':
                # Week-end/férié - ne casse PAS la séquence et ne compte pas
                # On ne fait rien, on continue
                pass
            else:
                # Présent ou télétravail - casse la séquence
                current_consecutifs = 0
...
def create_dynamic_calendar_sheet(wb, csv_file):
    ...
    # Sélecteur de collaborateur - VERSION SIMPLIFIÉE
    ws['A2'] = "Collaborateur :"
    ...
    # Créer une liste simple dans une zone cachée pour la validation
    # Au lieu d'utiliser une formule complexe, on crée une plage
    list_start_row = 1
    for idx, collab in enumerate(collaborateurs):
        ws.cell(row=list_start_row + idx, column=40, value=collab)  # Colonne AN (cachée)

    # Créer la validation de données avec référence à la plage
    list_range = f"$AN$1:$AN${len(collaborateurs)}"
    dv = DataValidation(type="list", formula1=list_range, allow_blank=False)
    ws.add_data_validation(dv)
    dv.add('B2')

    # Masquer la colonne avec la liste
    ...
    # IMPORTANT : On ne met PAS de formatage conditionnel
    # Les codes texte s'afficheront sans couleurs mais la liste déroulante fonctionnera
    print("  ⚠️  Pas de formatage conditionnel (pour compatibilité Excel Entreprise)")
    print("  ℹ️  Les codes seront affichés en texte : TV, CV, RV, etc.")
